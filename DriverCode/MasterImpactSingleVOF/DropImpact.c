...
#include "two-phase.h"               // interfacial flow propert setup and functionality
...
#include "tracer.h"                  // for tracer
#include "tag.h"                     // helps track droplet properties


// Initialise passive tracer for drop
scalar dTracer[];
...
#define poolHeight 3.0                 // Pool height (in radii)
#define domainSize 10.0                // Computational box size (in radii)
...
int main(int argc, char * argv[]) {
  ...
  // Pointer of the file to bubble count info
  {
    char name[200];
    sprintf(name, "logbubbles.dat");
    fp_bubbles = fopen(name, "w");
  }
...
event init (t = 0.0) {
  ...
  fraction (f, union(- x + filmHeight, sq(1.0) - sq(x - (filmHeight + 1.0 + 0.1)) - sq(y)));
  
  // Add passive tracer inside drop for visualisation
  fraction (dTracer, sq(1.0) - sq(x - (filmHeight + 1.0 + 0.1)) - sq(y));
...
event adapt (i++) {
  ...
  adapt_wavelet ((scalar *){f, dTracer, u}, (double[]){1e-6, 1e-3, 1e-3, 1e-3}, maxLevel, minLevel);
...
event saveInterfaces (t = 0.0; t += 0.01; t <= tEnd) {
    ...
    char nameInterfaces[200];

    sprintf(nameInterfaces,"Interfaces/interface-%0.2f.dat",t);

    FILE * fp1 = fopen(nameInterfaces, "w");
    output_facets (f, fp1);	
    fclose(fp1);
    
    /*char nameInterfacesDrop[200];

    sprintf(nameInterfacesDrop,"Interfaces/interfaceDrop-%0.2f.dat",t);

    FILE * fp2 = fopen(nameInterfacesDrop, "w");
    output_facets (dTracer, fp2);	
    fclose(fp2);*/
}

// Only early-stage pressure information required
event extractPressureData (t = 0.0; t += 0.1; t <= 2.01) {
...
// Bubble volume metrics
event bubbles (t += 0.001)
{
  scalar m[];
  foreach()
    m[] = (1.-f[]) > 1e-2;
  ...
  #endif
  for (int j = 0; j < n; j++)
    fprintf (fp_bubbles, "%d %g %d %g %g %g\n", i, t,
	     j, v[j], b[j].x/v[j], b[j].y/v[j]);
  fflush (fp_bubbles);
}
...
event movies (t += 0.001){
  ...
  view(width=1900, height=1050, fov=12.0, ty = 0.25, quat = { 0, 0, -0.707, 0.707 });
